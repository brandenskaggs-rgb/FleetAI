...
if (!DEV_SETUP) {
  console.error("[seed-dev] DEV_SETUP is not true. Refusing to seed.");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`[seed-dev] Seeded ${usersToSeed.length} users into ${filePath}`);
  Object.keys(generated).forEach((email) => {
    console.warn(`[seed-dev] Generated password for ${email}: ${generated[email]}`);
  });
}
